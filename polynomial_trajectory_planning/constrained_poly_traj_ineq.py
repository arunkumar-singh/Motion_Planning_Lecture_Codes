...
Mx_ineq = vstack(( P_xdot, -P_xdot, P_xddot, -P_xddot  ))
nx_ineq = hstack(( vx_max-dot(A_xdot, c_x_tilda), -vx_min+dot(A_xdot, c_x_tilda), ax_max-dot(A_xddot, c_x_tilda), -ax_min+dot(A_xddot, c_x_tilda)))

My_ineq = vstack(( P_ydot, -P_ydot, P_yddot, -P_yddot  ))
ny_ineq = hstack(( vy_max-dot(A_ydot, c_y_tilda), -vy_min+dot(A_ydot, c_y_tilda), ay_max-dot(A_yddot, c_y_tilda), -ay_min+dot(A_yddot, c_y_tilda)))


sol_x = solvers.qp(cvxopt.matrix(Qx, tc = 'd'), cvxopt.matrix(qx, tc = 'd'), cvxopt.matrix(Mx_ineq, tc = 'd'), cvxopt.matrix(nx_ineq, tc = 'd'), cvxopt.matrix(Mx_eq, tc = 'd'), cvxopt.matrix(nx_eq, tc = 'd'))
sol_x = array(sol_x['x']).squeeze()


sol_y = solvers.qp(cvxopt.matrix(Qy, tc = 'd'), cvxopt.matrix(qy, tc = 'd'), cvxopt.matrix(My_ineq, tc = 'd'), cvxopt.matrix(ny_ineq, tc = 'd'), cvxopt.matrix(My_eq, tc = 'd'), cvxopt.matrix(ny_eq, tc = 'd'))
